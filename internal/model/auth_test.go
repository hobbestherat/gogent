...
// TestKeyAuthHeaders covers the key-based auth scheme: each authMode places the
// key in the right place, static extraHeaders ride along, and an empty key
// contributes no auth header (only the extras).
func TestKeyAuthHeaders(t *testing.T) {
	cases := []struct {
		...
		{
			name:   "azure api-key",
			auth:   keyAuth{mode: authAzureKey},
			key:    "k4",
		...
		},
		{
			...
			auth:   keyAuth{mode: authQuery, queryParam: "key"},
			key:    "k5",
			absent: []string{"Authorization", "x-api-key", "api-key", "key"},
		},
		{
			name: "bearer with attribution headers",
			auth: keyAuth{mode: authBearer, extraHeaders: map[string]string{"HTTP-Referer": openRouterReferer, "X-Title": openRouterTitle}},
			key:  "k6",
		...
		},
		{
			...
			auth:   keyAuth{mode: authBearer, extraHeaders: map[string]string{"X-Title": openRouterTitle}},
			key:    "",
...
func TestAPIKeyRoundTripperQueryAuth(t *testing.T) {
	...
	client := &http.Client{Transport: &APIKeyRoundTripper{
		apiKey:     "g-secret",
		headers:    auth.headers("g-secret"),
		queryParam: auth.query(),
	}}
...
func TestAPIKeyRoundTripperAzureAuth(t *testing.T) {
	...
	client := &http.Client{Transport: &APIKeyRoundTripper{
		apiKey:     "az-secret",
		headers:    auth.headers("az-secret"),
		queryParam: auth.query(),
	}}
