...
- `ui/tui/sidebar_summary_issue510_test.go` (authoritative `statusBarLabel` pin):
  `allZeroBar` const; `TestStatusBarLabel` table wants; `TestStatusBarLabel_FixedOrder`
  order slice (`"‖1"`→`"⏸1"`); `TestStatusBarLabel_ExactlyTwoPipes` **comment only**
  (assertion of exactly two `|` bytes is unchanged — key regression guard, must still
  pass); `TestSummaryBar_AppliesOnEverySession` / lifecycle `check(...)` literals;
  `TestSyncFoldSuffixes_PipeSentinelNotWaitingGlyph` `wantBar` + comment; dismiss-failed
  assertions. `TestStatusBarLabel_NoIdleGlyph` is unchanged (still asserts no `•`).
...
within typical sidebar width. The implementation will eyeball this with `/run` or an
existing render integration test.
...
**(3) No regressions.** Two invariants are the load-bearing ones, both preserved:
  - *Pipe-sentinel contract.* `syncFoldSuffixes` re-derives the fold suffix by
    ...
    last `|`. Guarded by `TestStatusBarLabel_ExactlyTwoPipes` (assertion unchanged) and
    `TestSyncFoldSuffixes_PipeSentinelNotWaitingGlyph` (literal updated, purpose
    unchanged).
...
- `TestStatusBarLabel_ExactlyTwoPipes` and `TestSyncFoldSuffixes_PipeSentinelNotWaitingGlyph` pass.
...
- **Truncation at minimum sidebar width:** the +3 cells are expected to fit, but if a
  render test reveals truncation of the bar at the smallest configured sidebar width,
  that would be the only point needing follow-up (it would predate this change in spirit
  but be newly visible). Expected to be a non-issue.
